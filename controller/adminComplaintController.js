...
const replayComplaintByEmail = async (req, res) => {
  try {
    ...
    let transporter = nodemailer.createTransport({
      ...
      secure: true,
      auth: {
        user:"hala.almasri.s.2002@gmail.com",//process.env.USER, 
        pass:'waaf tsmf wbyf wdeg' // process.env.APP_PASSWORD,
      },
    });

    let mailOptions = {
      from:' "Idea Vest" <hala.almasri.s.2002@gmail.com>',// '"Idea Vest" <' + process.env.USER + '>',
      to: accusedEmail, 
    ...
    return res.status(200).json({ message: "Email sent successfully to " + accusedEmail });
