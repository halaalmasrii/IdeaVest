...
const createOpportunity = async (req, res) => {
  try {
    let {opportunityname, industry, fundingamount, reqfunding, description ,role } = req.body;
...
const getOpportunity = async (req, res )=> {
  ...
  const op = await Opportunity.find({role:role , isDeleted:false}).populate('user');
  return res.status(200).json({op})
}

const updateOpportunity = async (req, res) => {
  try {
    const {opportunityname, industry, fundingamount, reqfunding, description } = req.body;
    const opportunityId = req.params.id;
    const opportunity = await opportunity.findByIdAndUpdate(
      opportunityId,
      { opportunityname ,industry, fundingamount, reqfunding, description },
...
const getOpportunityByUser = async (req,res)=>{
  ...
  console.log(userId);
     ...
     opportunity = opportunity.filter(op=>{
      return op.user._id === id
     })
